...
| Two tabs open, garbled transcript, agents replying to the wrong conversation | Concurrent runs shared one mesh and one history; both runs interleaved and both poisoned provider memory | One run per browser session. A second request waits its turn in a bounded per-session queue (`202` with its position; `409` only when the operator disabled queueing with `--max-queued 0`, `429` when the queue is full) |
