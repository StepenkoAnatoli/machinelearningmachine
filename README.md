...
**Engineering Quality:**
- 🧪 700 tests (510 Python + 190 jsdom browser cases) running in CI on Python 3.10/3.11/3.12, plus ruff, `pip-audit`, a vendor-integrity check, and a job that installs the built wheel and *serves* it
...
node --test tests/js/*.test.mjs          # 190 jsdom browser tests (needs: npm ci)
...
│       ├── offline.test.mjs           # 18 tests: worker registration, the unreachable-server state (and the signals that must not trigger it), and what is disabled while it lasts
