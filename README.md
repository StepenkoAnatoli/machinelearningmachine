...
**Engineering Quality:**
- 🧪 470 tests (434 Python + 36 jsdom browser cases) running in CI on Python 3.10/3.11/3.12, plus ruff, `pip-audit`, a vendor-integrity check, and a job that installs the built wheel and *serves* it
...
node --test tests/js/*.test.mjs          # 36 jsdom browser tests (needs: npm ci)
...
│       └── client-lifecycle.test.mjs  # 21 tests: gap refetch, released session, 409/queue lifecycle, badges, retry-wait wording, reconnect recovery, windowed transcript
