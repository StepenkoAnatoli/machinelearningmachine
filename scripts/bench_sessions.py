...
Writes a directory of realistic saved transcripts (default: 50 files, 1000 messages
each - the scale a long multi-agent code conversation actually reaches) into a
temporary directory, then times two listings of it:
...
for the process, and the whole thing is deleted on exit.
    ...
    python scripts/bench_sessions.py --files 50 --messages 1000
...
def main(argv: list | None = None) -> int:
    ...
    parser.add_argument("--messages", type=int, default=1000, help="messages per transcript")
    parser.add_argument("--chars", type=int, default=1000, help="approx characters per message")
    ...
    try:
        ...
        print(
            f"{args.files} transcripts, {disk / 1e6:.1f} MB on disk "
